...
app.post("/create-payment", async (req, res) => {
    try {
        ...
        // Debug: Log key prefixes
        console.log("Using public key starting with:", SPACEPAY_PUBLIC_KEY.substring(0, 15) + "...");
        console.log("Using secret key starting with:", SPACEPAY_SECRET_KEY.substring(0, 15) + "...");

        // Create Spacepay payment
        const requestBody = {
            public_key: SPACEPAY_PUBLIC_KEY.trim(),
            secret_key: SPACEPAY_SECRET_KEY.trim(),
        ...
        console.log("Spacepay request body (keys masked):", { ...requestBody, public_key: "***", secret_key: "***" });

        const spacepayResponse = await axios.post(SPACEPAY_API_URL, requestBody);

        const paymentData = spacepayResponse.data;
        console.log("Spacepay response:", JSON.stringify(paymentData));
        ...
        if (paymentData && paymentData.payment_url) {
            ...
            console.error("Spacepay no payment_url:", paymentData);
            return res.status(500).json({ error: "Could not create payment" });
        }
    } catch (error) {
        console.error("Create payment error:", error.message);
        if (error.response) {
            console.error("Spacepay error status:", error.response.status);
            console.error("Spacepay error data:", JSON.stringify(error.response.data));
        }
        return res.status(500).json({ error: error.response ? `Spacepay error: ${error.response.status}` : "Internal server error" });
    }
});
